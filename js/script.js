...
async function main() {
        ...
        if (filterBar) {
            filterBar.addEventListener('click', (e) => {
                if (e.target.matches('.filter-btn')) {
                    ...
                    url.searchParams.set('category', category); // Keep category in URL
    ...
    async function initializeProductDetailPage() {
        ...
        if (!productId) {
            container.innerHTML = '<p>محصولی یافت نشد. <a href="products.html">بازگشت به محصولات</a>.</p>';
            return;
        }
        ...
        try {
            ...
            container.innerHTML = `
                <div class="product-detail-image-container"><img src="${product.image}" alt="${product.name}"></div>
            ...
            container.innerHTML = '<p>خطا در بارگذاری محصول.</p>';
        }
    }
    ...
    async function initializeCheckoutPage() {
        ...
        const { data: products, error } = await supabase.from('products').select('id, priceValue');
        if (error) {
            console.error("Could not fetch products for price calculation:", error);
            return;
        }
        ...
        form.addEventListener("submit", async (e) => {
            ...
            const detailedCartItems = cart.map(item => {
                ...
                if (product) {
                    ...
                    return { product_id: item.id, quantity: item.quantity, price_at_purchase: product.priceValue };
                }
                return null;
            }).filter(Boolean);
...
// js/script.js - REPLACE just this function
// js/script.js - ADD THIS MISSING FUNCTION
...
async function initializeAccountPage() {
    ...
    function initializeSearch() {
        ...
        if (!toggleBtns.length || !container || !closeBtn || !form) return;

        toggleBtns.forEach(btn => btn.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            container.classList.toggle('is-visible');
            if (container.classList.contains('is-visible')) input.focus();
        }));

        closeBtn.addEventListener('click', () => container.classList.remove('is-visible'));
        ...
        document.addEventListener('click', (e) => {
            const isToggleButton = e.target.closest('.search-toggle-btn');
            if (!container.contains(e.target) && container.classList.contains('is-visible') && !isToggleButton) {
    ...
    function initializeMobileMenu() {
        ...
        const openMenu = () => overlay.classList.add("is-active");
        const closeMenu = () => overlay.classList.remove("is-active");
    ...
    function lazyLoadFonts() {
        ...
        fontLink.href = "https://fonts.googleapis.com/css2?family=Vazirmatn:wght@300;400;700&display=swap";
